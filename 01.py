#!/usr/bin/env python
"""
PhysNetJax training on rMD17 with information-content based train/valid selection.

This script keeps the official rMD17 test split untouched, then builds train/valid
sets from the official train pool using either random sampling or ranked window
selection by an information metric such as gzip_bytes_cart, gzip_bytes_zmat, or
gzip_bytes_soap.
"""
...
def make_information_split(
    ...
    *,
    ...
    # If stride < window_size, prevent leakage through overlapping windows.
...
def make_selected_data(
    ...
    """Create train/valid data and metadata from one selection config."""
    ...
    if selection.kind == "random":
        ...
        if selection.train_fraction is None:
            # Match n_train / (n_train + n_valid) by default.
            selection_train_fraction = training.n_train / (training.n_train + training.n_valid)
        else:
            selection_train_fraction = selection.train_fraction
...
def train_one_experiment(
    *,
    ...
    print(f"Train energy mean subtracted: {train_e_mean:.10f}")
    print("=" * 80)

    # Persist split/index metadata before training.
...
def evaluate_test_set(
    *,
    ...
    print(
        f"Forces MAE={metrics['forces']['mae_kcal_mol_A']:.4f} kcal/mol/Å "
        f"({metrics['forces']['mae_ev_A']:.4f} eV/Å)"
    )
...
def load_experiment_data(config: ExperimentConfig) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any], Path, int]:
    ...
    official_train_pool, test_idx = load_rmd17_official_splits(
        splits_dir,
        config.dataset.split_id,
    )
    ...
    print(
        f"Official fold {config.dataset.split_id:02d}: "
        f"train pool={len(official_train_pool)}, test={len(test_idx)}"
    )
...
def collect_checkpoint_valid_losses(ckpt_dir: Path, every: int = 10) -> pd.DataFrame:
    """Collect validation losses from checkpoint directories for later comparison plots."""
...
def summarize_validation_curves(curves: dict[str, Iterable[float]]) -> pd.DataFrame:
    """Compute a compact summary of validation curves."""
    rows = []
